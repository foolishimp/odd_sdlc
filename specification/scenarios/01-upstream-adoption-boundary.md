...
**Purpose**: Prove that `odd_method` treats carried-forward source material as
bounded input rather than inherited live authority.

## Scenario

Bootstrap `odd_method` from a clean project-owned specification root while
...
- boundary path: `odd_method` refuses to inherit prior runtime/control-plane
  baggage by default
