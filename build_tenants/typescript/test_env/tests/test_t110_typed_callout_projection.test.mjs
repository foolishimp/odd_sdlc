...
test("T-129 installed odd_sdlc run keeps active worker alive past inactivity lease and archives ABG liveness projection", async () => {
  ...
  const start = await withEnv(
    {
      ...
      ODD_SDLC_WORKER_INACTIVITY_TIMEOUT_MS: "300",
  ...
  assert.equal(Array.isArray(liveness.probeCoverageRefs), true);
  assert.equal(liveness.probeCoverageRefs.length > 0, true);
  ...
  assert.equal(summary.inactivityTimeoutMs, 300);
});
