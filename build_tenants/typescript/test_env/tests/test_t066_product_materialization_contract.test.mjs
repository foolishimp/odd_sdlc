...
function writeTenantTechnologyStackSpec(
  ...
  const helloWorldRust =
    constraints.selectedOutputRoot.includes("hello_world_rust");
  const defaultSourceTargets = helloWorldJavascript
    ? ["src/hello.js"]
    : helloWorldRust
      ? ["src/main.rs"]
      : ["src/main/scala/generated/DataMapper.scala"];
  const defaultTestTargets = helloWorldJavascript
    ? ["test/hello.test.js"]
    : helloWorldRust
      ? ["tests/main.test.rs"]
      : ["src/test/scala/generated/DataMapperSpec.scala"];
  const sourceTargets = options.sourceTargets ?? defaultSourceTargets;
  const testTargets = options.testTargets ?? defaultTestTargets;
  if (
    ...
    `${JSON.stringify(
      {
        ...
        testingTechStack: {
          testRunner,
          ...(testTargets.length > 0 ? { testTargets } : {}),
          testRoots: [
...
function writeOutputSurface(manifest, title) {
  const componentDepthRegister = componentDepthRegisterForManifest(manifest);
  const artifact =
    componentDepthRegister === null
      ? `${[`# ${title}`, "", `edge: ${manifest.edgeName}`].join("\n")}\n`
      : `${JSON.stringify(componentDepthRegister, null, 2)}\n`;
  mkdirSync(dirname(manifest.outputFile), { recursive: true });
...
function tenantRelativeTargetForRole(manifest, role, fallback) {
  const selectedOutputRoot = manifest.productMaterialization.selectedOutputRoot
    .replace(/\\/g, "/")
    .replace(/\/+$/u, "");
  const target = declaredProductFileTargets(manifest).find((candidate) => {
    const relative = candidate.startsWith(`${selectedOutputRoot}/`)
      ? candidate.slice(selectedOutputRoot.length + 1)
      : candidate;
    return role === "test"
      ? /(^|\/)(test|tests)(\/|$)|\.test\./u.test(relative)
      : !/(^|\/)(test|tests)(\/|$)|\.test\./u.test(relative);
  });
  if (target === undefined) {
    return fallback;
  }
  return target.startsWith(`${selectedOutputRoot}/`)
    ? target.slice(selectedOutputRoot.length + 1)
    : target;
}

function componentDepthRegisterForManifest(manifest) {
  const base = {
    kind: "sdlc_component_depth_register",
    registerVersion: "ts-component-depth-v1",
    targetAssetType: manifest.targetAssetType
  ...
  if (manifest.targetAssetType === "component_code_surface") {
    const relativePath = tenantRelativeTargetForRole(
      manifest,
      "source",
      "src/main/scala/generated/DataMapper.scala"
    );
