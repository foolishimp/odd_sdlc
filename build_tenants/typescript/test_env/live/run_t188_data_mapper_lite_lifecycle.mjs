...
function parseSummary(stdout) {
  try {
    ...
    fail("T-188 data_mapper-lite runner did not emit parseable JSON summary", {
...
function assertLifecycleAnalysis(summary) {
  ...
  const reportPath = path.join(summary.archiveRoot, "t188_data_mapper_lite_lifecycle_analysis.md");
  ...
  if (missing.length > 0) {
    fail("T-188 data_mapper-lite lifecycle did not traverse the required edge order", {
      missing,
      edgeSequence,
  ...
  if (executionAttempt === undefined) {
    fail("T-188 data_mapper-lite did not admit a test execution result edge", {
      analysisReport: reportPath
    });
  }
  if (executionAttempt.executionEvidenceSource !== "graph_test_execution_result") {
    fail("T-188 data_mapper-lite execution evidence was not graph-generated", {
      executionEvidenceSource: executionAttempt.executionEvidenceSource,
      analysisReport: reportPath
    });
  }
  if (executionAttempt.executionEvidenceStatus !== "succeeded") {
    fail("T-188 data_mapper-lite execution evidence did not succeed", {
  ...
  if (
    ...
    fail("T-188 data_mapper-lite execution evidence lacks generated test counts", {
  ...
  if (analysis.currentStateTelemetrySummary.finalClosureDisposition !== "close") {
    fail("T-188 data_mapper-lite lifecycle did not close from admitted evidence", {
  ...
  const assertion = {
    kind: "t188_data_mapper_lite_lifecycle_assertion",
    archiveRoot: summary.archiveRoot,
    templateRoot: summary.templateRoot,
    selectedRequirementIds: DATA_MAPPER_LITE_SELECTED_REQUIREMENT_IDS,
    analysisReport: reportPath,
  ...
  writeFileSync(
    path.join(summary.archiveRoot, "t188_data_mapper_lite_lifecycle_assertion.json"),
...
function main() {
  ...
  const result = spawnSync(process.execPath, [RUNNER], {
    ...
    timeout: configuredLiveTimeoutMs(
      "ODD_SDLC_TS_T188_DATA_MAPPER_LITE_TIMEOUT_MS",
      RUNTIME_POLICY.liveHarnessLifecycleCommandTimeoutMs
    )
  });
  writeFileSync(
    path.join(PACKAGE_ROOT, "test_env/test_runs/t188_data_mapper_lite_last_runner_stdout.json"),
    result.stdout ?? "",
    "utf8"
  );
  writeFileSync(
    path.join(PACKAGE_ROOT, "test_env/test_runs/t188_data_mapper_lite_last_runner_stderr.log"),
    result.stderr ?? "",
    "utf8"
  );
  if (result.status !== 0) {
    fail("T-188 data_mapper-lite lifecycle runner failed", {
  ...
  if (summary.templateRoot !== DATA_MAPPER_LITE_FIXTURE_ROOT) {
    fail("T-188 data_mapper-lite runner did not use the lite fixture root", {
      observed: summary.templateRoot,
      expected: DATA_MAPPER_LITE_FIXTURE_ROOT
    });
  }
