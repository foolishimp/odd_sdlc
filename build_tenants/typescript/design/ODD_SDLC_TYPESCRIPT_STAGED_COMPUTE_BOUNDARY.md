...
| A1 | `replayEventsWithGraphContinuationCursor` formerly synthesized vector lifecycle events | ABG continuation and replay projection | W-110 slice D consumes ABI 4.1.0-rc.1 `applyExplicitGraphVectorResumeCursor(...)`; SDLC no longer constructs vector lifecycle cursor events | source negative test plus installed continuation replay |
...
| --- | --- | --- | --- | --- | --- |
| A1 | synthetic cursor event rows | done: SDLC construction deleted; ABI 4.1.0-rc.1 `applyExplicitGraphVectorResumeCursor(...)` consumed | ABG explicit resume cursor route | ABI 4.1.0-rc.1 installed and pinned | source negative test |
| A4 | `constructVector*`, `constructGraphSpan*`, `constructFdAuthority*` before append | done for production runtime authorship: F_D audit rows are projection-only; cursor and graph-span/reentry rows consume ABI routes | ABG/system routes plus `emit()` sink | construct-site inventory complete and ABI 4.1.0-rc.1 installed | construct-site test |
...
| repair graph-span reentry | prior `repairReentryGraphSpanRuntimeEvents` constructed schedule/assessment/foldback events plus plan/apply locally | `graph_span_evaluation_scheduled`, `graph_span_assessed`, `graph_span_foldback_evaluated`, `graph_reentry_planned`, `graph_reentry_applied` | **consumed in W-110 slice D**: ABI 4.1.0-rc.1 publishes `applyGraphSpanReentryRoute(...)`; SDLC submits admitted product assessment candidates and ABG returns authored graph-span/reentry events in runner ordering law | Keep source guard rejecting local graph-span and graph-reentry constructors in `installed_operator.ts`; SDLC may only submit product assessment candidates. |
...
flows through ABI 4.1.0-rc.1 routes.
