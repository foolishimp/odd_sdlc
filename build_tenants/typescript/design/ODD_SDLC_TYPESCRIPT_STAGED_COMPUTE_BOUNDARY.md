...
| A2 | installed multi-attempt loop authority ambiguity | partially done: public command/start surfaces deleted; residual installed-operator control code remains under T-204 audit | layered convergence moves to ABG command binding/control-loop surfaces; SDLC may retain only product plugin/session adapters with explicit survival proof | A5 transition-ref consumption complete in ABI 4.1.0-rc.7; T-204 source inventory pending | pack guard, source guard, focused start/runtime tests |
