...
| ABG CLI `start`, `start/*`, `workspace_api/*` | ABG command ingress plus product projection adapters | ABG admits operator request and enters runtime; odd_sdlc projects product policy/query surfaces behind that ingress | odd_sdlc package API dispatches workers, owns retry loops, or substitutes traversal meaning for ABG runtime truth |
