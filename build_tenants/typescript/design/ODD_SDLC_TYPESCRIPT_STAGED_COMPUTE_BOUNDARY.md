...
| A2 | `executeInstalledOperatorStartWithReentry` and `executeInstalledOperatorStart(...)` formerly owned local installed start/control | ABG command/control loop with SDLC as product plugin shell | T-204 state: public command ingress, start/gaps package APIs, and installed start executors are removed. Surviving `start/*` and `operator/installed_operator.ts` rows are classified product projection/plugin adapter surfaces in the source survival inventory: they are not exported as command/control, and source guards reject local retry, reentry, runtime-event authorship, and traversal-control ownership. | pack guard, source guard, focused start/runtime tests, and T-204 survival inventory |
...
| A2 | installed multi-attempt loop authority ambiguity | done: public command/start surfaces and installed start executors are deleted; surviving `start/*` and `operator/installed_operator.ts` rows are product projection/plugin-session adapters with explicit survival proof | layered convergence moves to ABG command binding/control-loop surfaces; SDLC retains only product plugin/session adapters with source-inventory proof | A5 transition-ref consumption complete in ABI 4.1.0-rc.10; T-204 source survival inventory closes the current code/src tree | pack guard, source guard, focused start/runtime tests |
