...
| Worker-controlled traversal recursion | former `code/src/operator/handoff.ts`; current worker transport and launch contract modules | replace | Worker control boundary forbids workers from invoking product-local or ABG command bindings, traversal, install, resume, or nested workers. | T-118 |
