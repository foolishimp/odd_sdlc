...
function compactDesignDepthPromptLineGroups(input: {
  ...
  return Object.freeze({
    preAuthorityLines: Object.freeze([
      ...
      "- After the component/file-target checkpoint, the evaluator must use bounded admitted evidence to advance one named semantic section: moduleSchemaFragments, moduleStateDiagramFragments, aggregateDomainModel, aggregateSunnyDaySequence, or satisfied verdict axes.",
      "- Repeating the checkpoint with empty semantic sections or partial verdict axes after bounded evidence is evaluator failure, not honest partial progress.",
      ...
      "- Component checkpoint writes fileTargetRows plus matching componentTopologyRows and componentRealizationRows before broad reads.",
...
function designDepthFpEvaluatorPromptLineGroups(input: {
  ...
  return Object.freeze({
    ...
    "- The content ledger file is the evaluation surface. Prefer file writes and bounded self-check reads over terminal narration.",
	    ...
	    "- Include stackProfileRows and implementationModuleRows in the minimum checkpoint. The checkpoint is a bounded liveness candidate only; final projection also requires non-empty schema/state/domain/sequence carriers and satisfied entity, attribute, and flow verdict axes.",
	    "- The next checkpoint after that must advance a named semantic section from bounded evidence. Do not create an all-empty full-section pressure map.",
    ...
    "- There is no framework-authored recipe for deriving the full register from authority: do not parse ADR tables by a fixed procedure for final sections; read the authority a section needs and decide that section content yourself.",
	    "- The first update must satisfy the named helper contract or an equivalent implementation. The minimum checkpoint packet is a bounded liveness candidate from admitted summary facts; F_P owns accepting, correcting, and refining it into final section values. F_D seeds draft scaffolding, packages evidence summaries, and admits fragment rows; final projection rejects shallow semantic floors instead of treating liveness as closure.",
    ...
    "- Do not spend the run enumerating every requirement id before writing the register. Use worker_result_report counts and high-signal samples for pressure; full trace remains in admitted evidence refs and source authority surfaces.",
	    "- If time or authority is insufficient, still write a structurally valid content ledger with partial or blocked designCompletenessVerdict axes and explicit reasons. Draft-row timeout is worse than an admitted pressure map with honest residual pressure, but partial or blocked axes are not closeable.",
