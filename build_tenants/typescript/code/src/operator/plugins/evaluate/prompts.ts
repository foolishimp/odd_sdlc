...
function tenantToolBoundaryPromptLines(
  ...
  return Object.freeze([
    ...
    "- Under the default Read/Write evaluator profile, treat tenant environment variables as declared context only; do not introduce command execution to inspect or mutate them.",
...
export function designDepthFpEvaluatorPrompt(input: {
  ...
  return [
    ...
    "- Tool-profile contract: the default design-depth evaluator process exposes only Read and Write. Do not plan around shell, Node, jq, rg, cat, sed, head, tail, grep, or other command helpers unless the active tool list explicitly exposes them.",
    ...
    "- If JSON or Markdown inspection is needed under the default Read/Write profile, use Read offset/limit and inspect only bounded line ranges.",
    ...
    "- Use only the active tool profile. Under the default Read/Write profile, summarize authority by bounded Read ranges and validate shape by bounded self-check reads; do not paste large source or register bodies into stdout.",
    "- Command-helper output budget before first evaluator update: command helpers are unavailable in the default Read/Write profile. If a future tool profile exposes one, the only pre-update helper allowed is the named carrier-helper update or equivalent same-path temp-then-rename implementation, and it may print only compact row counts.",
...
export function reviewGradeEdgeFulfillmentPrompt(input: {
  ...
  return [
    ...
    "Tenant tool boundary:",
    ...tenantToolBoundaryPromptLines(input.tenantToolEnvironment),
    "- Tool-profile contract: the default review-grade evaluator process exposes only Read and Write. Do not plan around shell, Node, jq, rg, cat, sed, head, tail, grep, scripts, local execution probes, or command helpers unless the active tool list explicitly exposes them.",
    ...
    "- Use only the active tool profile. Under the default Read/Write profile, inspect with bounded Read ranges and write the durable assessment JSON directly.",
    ...
    "- Do not execute commands unless the active tool list explicitly exposes command execution. Under the default Read/Write profile, evaluate declared executable or test execution contracts from admitted execution evidence; if required execution evidence is absent, mark the relevant obligation partial or blocked with execution_environment or semantic_not_realized instead of passing by inspection.",
