...
function buildInstructionContent(input: OddSdlcInstructionFilesInput): string {
  ...
  return [
    ...
    `- when the operator says \`start\`, run \`${oddSdlcCommand} start --workspace . --target next --until blocked\``,
    ...
    "- `start` may return `fp_worker_unattached`; that is a lawful blocked state, not a completed traversal",
    "- do not bypass `odd-sdlc-ts start` with ad hoc scripts when answering operator `start` requests",
    "",
    "## Interpretation Rule",
