...
### 6.8. Engine Public Invocation Contract (REQ-ENG)

These requirements define the top-level public API through which operators and integration tests invoke the CDME. Without this contract, downstream callers must reverse-engineer module internals, topology execution is ungoverned, and integration testing has no stable target.

* **REQ-ENG-001 (Top-Level Engine Entry Point):**
  **Priority:** Critical
  The engine MUST expose a top-level facade (`CdmeEngine`) that loads a topology configuration and returns an engine instance ready for dry-run or live execution.

  **Acceptance Criteria:**
  1. `CdmeEngine.load(topologyPath: String): CdmeEngine` — reads topology JSON from the given path and returns an initialized engine instance.
  2. The topology path is the sole required input; all module wiring (compiler, executor, accounting, fidelity, adjoint, assurance) is resolved internally.
  3. If the topology file cannot be read or fails schema validation, `load` MUST throw with a descriptive error before any execution is attempted.
  4. The returned instance is immutable with respect to topology; execution parameters are passed at call time.

* **REQ-ENG-002 (Dry Run Validation):**
  **Priority:** High
  The engine MUST support a dry-run mode that validates the topology and execution plan without writing output files or committing results.

  **Acceptance Criteria:**
  1. `engine.dryRun(): DryRunResult` — executes compilation and planning, returns a structured result.
  2. `DryRunResult` carries: `valid: Boolean`, `errors: Seq[String]`, `warnings: Seq[String]`, `planSummary: String`.
  3. A dry run MUST NOT write to any output path.
  4. A dry run MUST exercise the same compiler and type-resolution paths as a live run (REQ-AI-03).
  5. If `valid == false`, the caller has sufficient information to identify and fix the topology before live execution.

* **REQ-ENG-003 (Dataset Execution and Output Contract):**
  **Priority:** Critical
  The engine MUST execute against a dataset path and write outputs to a declared output base directory.

  **Acceptance Criteria:**
  1. `engine.run(datasetPath: String, outputBase: String): RunResult` — executes the full pipeline against partitioned JSONL input and writes outputs.
  2. `RunResult` carries boolean attestation fields for each processing stage (morphism compilation, type resolution, grain checking, accounting, fidelity, adjoint capture, assurance validation) plus overall `success: Boolean`.
  3. `outputBase/ledger.json` and `outputBase/processing_report.json` MUST be written on every run, regardless of pass or fail status.
  4. Results are also written to `outputBase/../results/` for integration test discovery.
  5. The method signature and output paths are stable across engine versions; breaking changes require a new major version.

* **REQ-ENG-004 (Ledger Output Contract):**
  **Priority:** Critical
  The `ledger.json` output produced by each run MUST conform to a machine-readable schema sufficient to prove the accounting invariant (REQ-ACC-01 through REQ-ACC-05).

  **Acceptance Criteria:**
  1. `Ledger.read(path: String): Ledger` — parses `ledger.json` from the given path.
  2. `Ledger` carries: `inputCount: Long`, `processedCount: Long`, `filteredCount: Long`, `errorCount: Long`, `balanced: Boolean`, `discrepancy: Option[String]`.
  3. `balanced == true` IFF `inputCount == processedCount + filteredCount + errorCount`.
  4. The schema is documented in this specification and validated by the `F_D` assurance surface at test time.
  5. Ledger is written atomically (write-to-temp + rename) to prevent partial reads.

* **REQ-ENG-005 (Processing Report Output Contract):**
  **Priority:** High
  The `processing_report.json` output MUST carry per-partition execution telemetry sufficient to diagnose morphism failures and fidelity gaps.

  **Acceptance Criteria:**
  1. `ProcessingReport.read(path: String): ProcessingReport` — parses `processing_report.json`.
  2. `ProcessingReport` carries: `partitions: Seq[PartitionProfile]`, `morphismTelemetry: Seq[MorphismTelemetry]`, `hopReports: Seq[HopReport]`.
  3. `PartitionProfile` contains: partition key, input record count, output record count, error count, execution duration.
  4. `MorphismTelemetry` contains: morphism identifier, input count, output count, failure count.
  5. `HopReport` contains: hop identifier, source entity, target entity, cardinality observed, cardinality budget, budget exceeded flag.
  6. The report is written immediately after execution completes, before ledger verification.

* **REQ-ENG-006 (Deterministic Reproducibility):**
  **Priority:** High
  Given identical topology, dataset, and configuration inputs, the engine MUST produce bit-identical `ledger.json` and `processing_report.json` outputs across runs.

  **Acceptance Criteria:**
  1. Record counts and all numeric fields in ledger and processing report are identical across repeated runs on the same inputs.
  2. No execution timestamp or run-ID field is embedded in the accounting ledger or processing report (timestamps belong in OpenLineage events, not in the proof artifacts).
  3. This property supports the `F_D` deterministic evaluator: the integration test suite compares output hashes against a baseline run.

**Physical Dataset Binding Contract:**

The engine's physical binding layer MUST resolve JSONL datasets using the following partition layout:

```
{datasetPath}/
  {date}/              # ISO-8601 date partition, e.g. 2024-01-01
    journeys.jsonl     # Booking-level records
    segments.jsonl     # Segment-level records
    customers.jsonl    # Passenger-level records
```

* **REQ-ENG-007 (Partitioned JSONL Physical Binding):**
  **Priority:** Critical
  The PDM binding MUST declare the partition key field name and the per-partition file layout. The engine MUST enumerate partitions by scanning the dataset root directory and processing each date-keyed subdirectory as an independent partition.

  **Acceptance Criteria:**
  1. The engine discovers partitions by listing immediate subdirectories of `datasetPath`; subdirectory names are the partition keys.
  2. Within each partition, the engine reads the declared entity files (`journeys.jsonl`, `segments.jsonl`, `customers.jsonl` or equivalents from PDM config).
  3. A missing partition file is a structural error routed to the Error Domain (REQ-TYP-03), not a silent skip.
  4. Partition enumeration is deterministic (lexicographic order) to satisfy REQ-ENG-006.
  5. The PDM binding artifact MUST declare file names per entity; hard-coded file names in engine source are not permitted.

**Integration with Existing Requirements:**

* Implements **REQ-AI-03** (Dry Run) as a first-class API method with a declared return type.
* Fulfils **REQ-ACC-02** (Accounting Ledger) output contract with `Ledger.read()` as the canonical consumer interface.
* Extends **REQ-INT-03** (Traceability) — the processing report is the per-run execution trace.
* Enforces **REQ-PDM-01** (Physical Storage Alignment) — partition layout is declared in PDM, not assumed by engine internals.

---

### 6.9. Build Tenant Contracts (REQ-BT)

A **build tenant** is a declared realization of the CDME topology for a specific execution platform. The same logical topology (nodes, morphisms, type system, accounting, fidelity) MUST be fully realizable by any conforming build tenant. Build tenants differ in execution technology; they do not differ in the obligations they satisfy.

Two build tenants are defined: `scala_spark` and `dbt`. A workspace MUST declare exactly one active build tenant. The pipeline generates build artifacts and test surfaces appropriate to that tenant.

---

#### REQ-BT-001: Build Tenant Declaration

**Priority:** Critical

Every workspace MUST declare an active build tenant in `project_constraints.yml`. The declaration MUST specify the tenant identifier, execution tier, runner mechanism, and capability contracts. The pipeline treats the declared tenant as authoritative for code generation, test surface selection, and obligation carry checks.

**Acceptance Criteria:**
1. `project_constraints.yml` contains an `active_tenant` field naming one of the defined tenants.
2. The declared tenant has a corresponding entry in `build_tenants` with `execution_tier`, `runner`, and `capability_contracts` fully specified.
3. If the active tenant is `scala_spark`, the pipeline enforces Spark-tier obligations (REQ-BT-002).
4. If the active tenant is `dbt`, the pipeline enforces DBT-tier obligations (REQ-BT-003).
5. A workspace with no declared tenant MUST fail pipeline intake triage with a descriptive error before any generation.
...
The `scala_spark` build tenant realizes the CDME topology as a multi-module SBT project using Scala 2.13 and Apache Spark 3.5. All data-plane operations MUST use the Spark DataFrame API; pure Scala collection processing is not permitted for production data paths.

**Acceptance Criteria:**
1. A `SparkSession` MUST be constructed at engine startup and passed to all data-reading and transformation components. Engine source code MUST NOT use `scala.io.Source` or JVM file I/O for dataset reads.
2. Physical bindings (REQ-ENG-007) MUST be resolved via `spark.read.json(path)` or `spark.read.text(path)` with explicit schema; the DataFrame is the carrier type for all record-level processing.
3. The tenant MUST provide a `CdmeEngineRunner` main object that accepts CLI arguments `--topology <path> --dataset <path> --output <path>` and invokes `CdmeEngine.load(topology).run(dataset, output)`.
4. `CdmeEngineRunner` MUST be packageable as a fat JAR via `sbt assembly` and executable as `spark-submit --class cdme.engine.CdmeEngineRunner cdme-engine.jar <args>`.
5. All morphism transformations (explode, compute, lookup) MUST be expressed as Spark DataFrame column expressions or UDFs, not as `map`/`flatMap` over Scala collections.
6. Partition discovery MUST use `spark.read` with a path glob or `listFiles` only as a partition-key enumerator; the data read itself MUST be via Spark.
7. The `cdme-engine` SBT module MUST declare `"org.apache.spark" %% "spark-sql" % sparkVersion` as a `provided` dependency.
8. Unit tests (ScalaTest) MAY use a local `SparkSession` with `master("local[*]")`; integration tests MUST use the same session configuration as production.

**Out of scope for this tenant:** DBT models, YAML schema tests, SQL morphism definitions.
...
The `dbt` build tenant realizes the CDME topology as a DBT project. Each morphism in the topology MUST have a corresponding DBT model. The type system, accounting invariant, and fidelity contracts MUST be enforced via DBT schema tests and custom generic tests.

**Acceptance Criteria:**
1. The DBT project MUST contain one `.sql` model per declared morphism; the model file name MUST match the `morphism_id` from the topology.
2. Physical bindings (REQ-ENG-007) MUST be declared as DBT `sources` with explicit column definitions and freshness constraints; hard-coded table references in model SQL are not permitted.
3. The type system (REQ-TYP-001 through REQ-TYP-007) MUST be enforced via DBT `schema.yml` column-level tests: `not_null`, `accepted_values`, and custom generic tests for type conformance.
4. The accounting invariant (REQ-ACC-001: `records_in = records_aggregated + records_filtered + records_errored`) MUST be enforced as a DBT singular test that queries the final model and asserts balance.
5. The fidelity certificate (REQ-COV-004) MUST be realized as a DBT model that computes per-hop null rates and type conformance rates, with a schema test asserting thresholds.
6. The Error Domain (REQ-TYP-003) MUST be realized as a separate DBT model (`error_domain`) populated via CASE-WHEN logic in morphism models; failed records MUST NOT be silently dropped.
7. The tenant MUST provide a runner script `run_dbt.sh --topology <path> --profile <profile> --target <target>` that invokes `dbt run && dbt test` and writes a `ledger.json` and `processing_report.json` conforming to REQ-ENG-004 and REQ-ENG-005 by querying model row counts post-run.
8. DBT lineage (the DAG) satisfies REQ-TRV-001 (traversal path) and REQ-TRV-002 (correct traversal order); the tenant MUST NOT introduce model dependencies that contradict the declared morphism chain in the topology.
9. The tenant targets a SQL-compatible warehouse (DuckDB for local, Snowflake or BigQuery for cluster); the warehouse type MUST be declared in `project_constraints.yml` under `capability_contracts.warehouse`.

**Out of scope for this tenant:** SparkSession, SBT modules, Scala source files.

---

#### REQ-BT-004: CLI Runner Contract (Both Tenants)

**Priority:** Critical

Every build tenant MUST expose a CLI runner conforming to this contract, regardless of execution technology. The runner is the governed integration test entry point.

**Acceptance Criteria:**
1. The runner accepts `--topology <path>`, `--dataset <path>`, and `--output <path>` as named arguments.
2. On success, the runner writes `<output>/ledger.json` and `<output>/processing_report.json` conforming to REQ-ENG-004 and REQ-ENG-005, then exits with code 0.
3. On failure, the runner writes whatever output was produced (partial ledger is permitted), writes a `<output>/run_error.json` with `{ "error": "<message>", "stage": "<stage>" }`, and exits with a non-zero code.
4. The runner is idempotent: running it twice on the same inputs overwrites prior output; it does not accumulate or append.
5. The runner is invokable from a shell without IDE tooling: `./run.sh <args>` or `spark-submit <jar> <args>` or `dbt run && ./emit_ledger.sh`.
6. Integration tests (test suite, UAT, CI) MUST invoke the runner via CLI, not by calling engine internals directly.

---

**Integration with Existing Requirements:**

* REQ-BT-002 fulfils the Spark platform declaration in `project_constraints.yml` and makes REQ-ENG-003 runnable outside test harness.
* REQ-BT-003 extends REQ-TRV-001 (traversal lineage) to a SQL/DBT execution surface.
* REQ-BT-004 provides the stable external interface that REQ-ENG-004/005 output contracts attach to.
* Both tenants satisfy the same REQ-ACC-*, REQ-COV-*, REQ-ADJ-*, REQ-TYP-* obligations; the tenant contract governs HOW, not WHETHER.
...
* **Requirement (Article 14 - Human Oversight):**
  Satisfied by **REQ-AI-01**, **REQ-AI-02**, and **REQ-AI-03**. The dry-run contract (**REQ-ENG-002**) provides a governed pre-execution gate.
* **Requirement (Article 15 - Robustness):**
  Satisfied by topological strictness preventing invalid mappings and guaranteeing determinism (**REQ-LDM-03**, **REQ-TYP-06**, **REQ-TRV-05**, **REQ-ENG-006**).
...
14. **Engine Invocation Contract:** A caller can load a topology, execute a dry run, and execute a live run against a partitioned JSONL dataset using only the `CdmeEngine` public facade, without referencing internal module types (REQ-ENG-001 through REQ-ENG-007). The resulting `ledger.json` proves the accounting invariant and `processing_report.json` provides per-partition telemetry, both readable via their declared `read(path)` constructors (REQ-ENG-004, REQ-ENG-005).
15. **Deterministic Output Hashing:** Running the engine twice against the same airline-clean dataset produces byte-identical `ledger.json` and `processing_report.json`, enabling `F_D` hash comparison in the integration test suite (REQ-ENG-006).
