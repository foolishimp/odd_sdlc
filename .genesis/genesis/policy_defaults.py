...
def execute_proof_policy(
    concern: Mapping[str, Any],
    *,
    assessments: list[Mapping[str, Any]] | tuple[Mapping[str, Any], ...],
) -> dict[str, Any]:
    """Execute the materialized proof concern and return a decision record."""
    mode = str(concern.get("mode") or "")
    assessment_rows = [dict(assessment) for assessment in assessments]
    if mode == "rerun_after_fp":
        passed = bool(assessment_rows) and all(assessment.get("result") == "pass" for assessment in assessment_rows)
        reason = "all_assessments_passed" if passed else "assessment_failure_detected"
    else:
        passed = bool(assessment_rows) and all(assessment.get("result") == "pass" for assessment in assessment_rows)
        reason = "default_truthy_pass_set" if passed else "default_failure"
    return {
        "mode": mode,
        "passed": passed,
        "reason": reason,
...
def execute_closure_policy(
    concern: Mapping[str, Any],
    *,
    proof_decision: Mapping[str, Any],
) -> dict[str, Any]:
    """Execute the materialized closure concern and return a decision record."""
    mode = str(concern.get("mode") or "")
    config = _mapping(concern.get("config"))
    proof_passed = bool(proof_decision.get("passed"))
    if mode == "resolve_or_escalate_fh":
        force_fail = bool(config.get("force_fail"))
        passed = proof_passed and not force_fail
        if not proof_passed:
        ...
        passed = proof_passed and not bool(config.get("force_fail"))
        reason = "default_closure_pass" if passed else "default_closure_failure"
    return {
        "mode": mode,
        "passed": passed,
        "reason": reason,
