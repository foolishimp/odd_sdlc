...
- priority: high
- dependencies: odd_sdlc B-020 active as the fulfillment-truth migration lane; related ABIogenesis work on start/gaps truth should remain substrate-owned
- intake_source: `data_mapper.test35` forensic analysis 2026-04-19; missing test-execution capability truth existed in normalization and ambiguity state but was not surfaced consistently in graph publication, `gaps`, or archive-edge diagnostics
...
`data_mapper.test35` exposed one real domain bug, but it appeared through
...
Claude split that one defect into three follow-on tickets:
