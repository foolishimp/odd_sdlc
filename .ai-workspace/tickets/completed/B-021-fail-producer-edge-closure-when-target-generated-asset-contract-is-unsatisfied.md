...
- priority: critical
- severity: sev-1
- intake_source: producer-edge forensic run 2026-04-19
...
Producer-edge deterministic closure now includes the current produced generated
surface contract, so a producing edge does not converge while its own output is
structurally invalid.

The landed closure path is:

- producer-edge deterministic checks include produced generated assets in
  `fd_checks.py`
- the generic deterministic runner fail-closes on
  `assess_generated_asset_contract(...)`
- constructor and live runtime now enforce the same target-contract law

Direct proof exists in:
...
The original ticket statement was true at intake time and is no longer true.

The produced target asset is now part of producer-edge deterministic closure.
What remains after this ticket is not producer-edge contract parity; it is the
separate operator-projection classification defect tracked by `B-019`.
...
This produced a false convergence chain:
...
One reproduced failure workspace showed the defect directly:
...
- truthful inherited-workspace qualification
...
- a fresh reproduced failing workspace keeps the failing producer edge open when
  the generated file is missing its required heading or marker text
